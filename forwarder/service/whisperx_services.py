...
def _get_whisper_model():
    ...
    if whisper_model is None:
        try:
            ...
            device = get_config("whisper.device", "cuda")
            compute_type = get_config("whisper.compute_type", "float16")
            download_root = get_config("whisper.download_root", None)
            whisper_model = whisperx.load_model(model_type, device=device, compute_type=compute_type,
                                                download_root=None,
...
def transcribe_audio(file_path: str, min_speakers: int, max_speakers: int) -> str:
    ...
    try:
        ...
        logger.info(f"音频文件加载完成，音频长度: {len(audio)}，开始转写...")
        # 1.执行转写
        ...
        result = remove_words_from_segments(result)
        ...
        print("hf_token", get_config("diarization.hf_token"))
        print("当前 token:", hf_token[:8] + "...")
        api = HfApi()
        whoami = api.whoami(token=hf_token)
        print("HuggingFace 用户信息:", whoami)
        device = get_config("whisper.device", "cpu")
        diarize_model = whisperx.diarize.DiarizationPipeline(use_auth_token=hf_token, device=device)
        # add min/max number of speakers if known
        if 0 < min_speakers <= max_speakers and max_speakers > 0:
            logger.info(
                f"使用已知的说话人数量范围进行说话人分离，最少说话人: {min_speakers}, 最多说话人: {max_speakers}")
...
def synthesize(audio_url: str, message_data: dict, config_path="config.yml") -> str:
    """
...
def remove_words_from_segments(result: dict) -> dict:
    """
    移除result['segments']中每个分段的'words'字段，返回新的result
    Args:
        result (dict): WhisperX转写结果，包含'segments'数组
    Returns:
        dict: 移除words后的result
    """
    segments = result.get('segments', [])
    for seg in segments:
        if 'words' in seg:
